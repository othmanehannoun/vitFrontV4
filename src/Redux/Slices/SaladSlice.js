     ...
     addToBase (state, { payload }) {
      const { _id } = payload;
        ...
        if(find){
          ...
          state.Base = [payload, ...state.Base];
        }
     
      },

      addToIngredient(state, { payload }) {
        const { _id } = payload;
            ...
            if(find){
              ...
              state.Ingredient = [payload, ...state.Ingredient];
            }

            // if(state.Ingredient.length > 5){
            //   state.price += 6
            // }
      },

      addToToppings(state, { payload }) {
        const { _id } = payload;

            const find = state.Toppings.find((item) => item._id === _id);
            
            if(find){
              state.Toppings = state.Toppings.filter(cartItem => cartItem._id !== _id)
            }
            else{
              state.Toppings = [payload, ...state.Toppings];
            }

      },

      addToSauce(state, { payload }) {
        const { _id } = payload;

            const find = state.Sauce.find((item) => item._id === _id);
            
            if(find){
              state.Sauce = state.Sauce.filter(cartItem => cartItem._id !== _id)
            }
            else{
              state.Sauce = [payload, ...state.Sauce];
            }
...
export const { reset, addToBase, addToIngredient, addToToppings, addToSauce} = saladCart.actions
